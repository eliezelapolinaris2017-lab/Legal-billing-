...
function applySettings(){
  ...
  $('brandMark').textContent=(settings.appInitials||'DP').slice(0,3).toUpperCase();
...
function loadSettings(){['appName','appInitials','bizName','bizPhone','bizEmail','bizAddress','invoicePrefix','quotePrefix','collectionPrefix','legalNote'].forEach(id=>$(id).value=settings[id]||'');Object.keys(baseTheme).forEach(id=>$(id).value=settings[id]||baseTheme[id])}
